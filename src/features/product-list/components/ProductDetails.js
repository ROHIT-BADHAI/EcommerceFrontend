...
const highlights = ["Best product", "cheap rated"];
...
export default function ProductDetails() {
  ...
  const handleCart = (e) => {
    e.preventDefault();
    const tempItem = { quantity: 1, product:product.id, user:user.id };
    delete tempItem["id"];
    dispatch(addToCartAsync(tempItem));
  };
  ...
  // useEffect(() => {
  //   if (cart.length>0 && cart.findIndex((cartItem) => cartItem.productId === product.id) < 0) {
              ...
              <form className="mt-10">
                ...
                {!alreadyInCart ? (
                  ...
                  <Link
                    ...
                    to="/cart"
                  >
                    Already Added to cart. Go to Cart?
                  </Link>
                )}
              </form>
            </div>
